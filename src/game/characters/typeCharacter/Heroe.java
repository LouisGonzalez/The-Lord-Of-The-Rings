...
public abstract class Heroe extends Character {
    ...
    private final int LAST_VALUE_DICE = 100;
    private Terminal terminal = new Terminal();

    public Heroe(String name, int life, int armor){
    ...
    public void attackTurn(Character enemy){
        int valueDice1 = dice.turnDice(FIRST_VALUE_DICE, LAST_VALUE_DICE);
        int valueDice2 = dice.turnDice(FIRST_VALUE_DICE, LAST_VALUE_DICE);
        this.setDamage(dice.verifyTwoDices(valueDice1, valueDice2));
        particularMove(enemy);
        if(validator.verifyDamage(this, enemy)){
            enemy.setLife(this.getDamage() - enemy.getArmor());
            enemy.setArmor(0);
            terminal.showMessage("*****************************************************");
            terminal.showMessage(enemy.getName() + " of the batallion of the beasts has received "+this.getDamage());
            validator.verifyStatusCharacter(enemy);
            if(!enemy.getIsAlive()){
                terminal.showMessage(enemy.getName() + " of the batallion of the beasts has died");
            }
            terminal.showMessage("*****************************************************");
        }
